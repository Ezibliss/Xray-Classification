...
# This is typically at model.features[0][0]
conv0 = model.features[0][0]  # Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False)
new_conv = nn.Conv2d(
    in_channels=1,
...
# Copy weights: average pretrained RGB weights to grayscale
with torch.no_grad():
    new_conv.weight = nn.Parameter(conv0.weight.mean(dim=1, keepdim=True))

# The layer
model.features[0][0] = new_conv

# Replace classifier
num_features = model.classifier[1].in_features  # model.classifier is Sequential
...
# Saving the model
torch.save(model.state_dict(), 'effnetmodel.pth')
print('Model saved as effnetmodel.pth')
#model_path = 'effnetmodel.pth'
